...
- [x] Select leading targets for a literature and druggability case study: **`IFI6`, `LY6E`, `HPRT1`, `KIF2C`** (user's choice, four instead of the plan's suggested one–two). `IFI6` and `LY6E` are the top 2 by composite score (1st and 2nd); `HPRT1` isn't (5th) but is the project's clearest tractability-vs-evidence contrast case — HPA's `Protein class` field flags it `FDA approved drug targets` despite having the *worst* essentiality rank of the 15 candidates. **Correction to an earlier note here:** that label isn't "already druggable" in the sense first assumed — `HPRT1` isn't allopurinol's target (allopurinol inhibits xanthine oxidase, a different enzyme); the FDA-approved-drug connection is that `HPRT1` metabolically activates the thiopurine prodrugs 6-mercaptopurine/6-thioguanine/azathioprine, not that an approved drug inhibits `HPRT1` itself. `KIF2C` was added on further review of the Week 2 essentiality filter: that filter only ever tested *differential* essentiality (TNBC vs. other DepMap lines), never absolute dependency magnitude. Rechecked directly against this project's own 25-line TNBC set: `KIF2C` has by far the strongest real dependency (median Chronos effect -0.487, 80% of TNBC lines dependent at ≤-0.30), while `IFI6`/`LY6E`/`HPRT1` are all weak on this same measure (medians -0.046/-0.037/-0.030, dependency called in ≤12% of lines) — those three still have real cases on other axes (tumor overexpression, survival, literature), but `KIF2C` is the one candidate combining relative selectivity with genuine absolute dependency. Full write-up (incl. a real TNBC-specific hit for `HPRT1` and a real TNBC-subtype gap for `IFI6`) in `case_study_research.md`.
- [x] Literature + druggability research for `IFI6` and `HPRT1` done (`LY6E`/`KIF2C` already covered in Weeks 2–3) — see `case_study_research.md`. Key findings: `IFI6`'s published breast-cancer prognostic signal is concentrated in ER+/PR+/HER2+/node-positive disease, with one 2025 paper explicitly finding **no significant TNBC association** — a real gap against this project's own TNBC-specific convergent evidence, stated plainly rather than glossed over. `HPRT1` has the opposite pattern: a 2024 pan-cancer paper found RNA "significantly elevated... particularly in basal cells and triple-negative breast cancer," validated against an independent TNBC dataset (GSE107764) — the most directly TNBC-specific literature hit of the three genes, despite `HPRT1`'s weak DepMap essentiality rank. Real oncogenic mechanism found too: a HIF-1α–`HPRT1` axis drives tumorigenesis/gefitinib resistance in EGFR-mutant lung cancer, and `HPRT1` becomes conditionally essential under mitochondrial ETC stress (plausible reconciliation for its weak *unperturbed* DepMap dependency — a hypothesis, not something this project's data confirms).
...
- [x] Pin the environment: `environment.yml` now pins Python to 3.10.21 and every direct dependency to an exact version (`jupyter` and `nbconvert` were previously unpinned or missing), and `environment.lock.txt` holds the full `pip freeze`. Verified by building a fresh environment from the yml: Python and all direct packages matched exactly, while 8 transitive packages (e.g. `tornado`, `urllib3`) had drifted to newer patch versions, which is the reason for keeping the lock file.
